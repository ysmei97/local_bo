...
T = 1  # default 1
epsilon = 0.1
neighbor_threshold = 0.8  # default 0.8
alpha = 10  # default 10
length_scale = 0.1  # default 0.5
...
'''
x_star = [np.array([0, 2]), np.array([4, 0]), np.array([-3, -1])]
x1_sample = np.linspace(-5, 5, sample_point)
x2_sample = np.linspace(-5, 5, sample_point)
x_sample = np.meshgrid(x1_sample, x2_sample)
# print('Initials:', x_star, 'Values:', griewank(np.array(x_star)))

fig = plt.figure(figsize=(10, 8))
ax1 = fig.add_subplot(2, 1, 1, projection='3d')
ax2 = fig.add_subplot(2, 1, 2, projection='3d')
ax1.plot_surface(x_sample[0], x_sample[1],
                 np.array(griewank(np.dstack((x_sample[0], x_sample[1])).reshape(-1, 2))).reshape(sample_point, -1),
...
anim.save('2d_griewank_pi_lpt1_new.gif', writer='pillow', fps=1, dpi=300)
