...
dimension = 3
iteration = 200
sample_point = 101  # default 400
x_star_num = 3
T = 1  # default 1
epsilon = 0.1
neighbor_threshold = 0.6  # default 0.8
alpha = 10  # default 10
length_scale = 0.5  # default 0.5
...
def local_optima_pi(x_star, x_sample):
    ...
    for i in range(sample_point):
        for j in range(sample_point):
            for k in range(sample_point):
                xx = np.array([x_sample[0][i][j][k], x_sample[1][i][j][k], x_sample[2][i][j][k]])
                ...
                mean = mean_.T @ kernel_pseudo_inverse @ griewank(np.array(x_star))
...
def animate(i):
    ...
    ax1.set_title("Griewank Function. Iteration: %s" % str(i + 1), size=10)
    ax2.set_title("Probability of improvement", size=10)
...
x_star = [np.array([0, 2, 1]), np.array([4, 0, 3]), np.array([-3, -1, -2])]
x1_sample = np.linspace(-5, 5, sample_point)
x2_sample = np.linspace(-5, 5, sample_point)
x3_sample = np.linspace(-5, 5, sample_point)
x_sample = np.meshgrid(x1_sample, x2_sample, x3_sample)

print('PI, sample101, d0.6')
...
for i in range(iteration):
    ...
    x1_ = griewank(np.array(x_star)[-1:] - np.array([0.1, 0, 0]))
    x1__ = griewank(np.array(x_star)[-1:] + np.array([0.1, 0, 0]))
    x2_ = griewank(np.array(x_star)[-1:] - np.array([0, 0.1, 0]))
    x2__ = griewank(np.array(x_star)[-1:] + np.array([0, 0.1, 0]))
    x3_ = griewank(np.array(x_star)[-1:] - np.array([0, 0, 0.1]))
    x3__ = griewank(np.array(x_star)[-1:] + np.array([0, 0, 0.1]))

    print('Step: %s' % str(i + 1), 'new observation: ', np.array(x_star)[-1:], 'value: ', f_x_star[-1:],
          'max acquisition value: ', np.max(acquisition_value), 'min acquisition value: ', np.min(acquisition_value))
    print('> neighbors: ', f_x_star[-1:] > x1_, f_x_star[-1:] > x1__, f_x_star[-1:] > x2_, f_x_star[-1:] > x2__,
          f_x_star[-1:] > x3_, f_x_star[-1:] > x3__, 'neighbor values: ', x1_, x1__, x2_, x2__, x3_, x3__)
    if (bool(f_x_star[-1:] > x1_) and bool(f_x_star[-1:] > x1__) and bool(f_x_star[-1:] > x2_) and bool(f_x_star[-1:] > x2__) and bool(f_x_star[-1:] > x3_) and bool(f_x_star[-1:] > x3__)):
        print('LOCAL Maximum')
